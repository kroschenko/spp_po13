...
def repeat(pattern, repeat_count):
    ...
    # Если количество повторений 0, возвращаем пустую строку
    if repeat_count == 0:
        return ""
...
# ============================================
# Тесты для метода repeat (ИСПРАВЛЕННАЯ ВЕРСИЯ)
# ============================================
...
class TestRepeat:
    ...
    def test_repeat_with_spaces(self):
        ...
        # Проверяем длину: 5 символов * 2 = 10 символов
    ...
    def test_repeat_with_spaces_detailed(self):
        ...
        # Проверяем, что это действительно повторение паттерна
        assert result == pattern + pattern
        assert result == " ABC  ABC "

        # Проверяем, что начинается и заканчивается правильно
        assert result.startswith(" ")
        assert result.endswith(" ")

        # Проверяем, что в середине два пробела подряд
        assert "  " in result
    ...
    def test_repeat_both_spaces(self):
        ...
        # Это ключевой тест, показывающий правильное поведение
        pattern = " ABC "
    ...
    def test_repeat_preserves_original_pattern(self):
        ...
        assert pattern == "test"  # оригинал не изменился
    ...
    def test_repeat_mutable_pattern_copy(self):
        ...
        assert result is not pattern  # разные объекты

    def test_repeat_with_whitespace_only(self):
        """Паттерн только из пробелов"""
        assert repeat("   ", 2) == "      "  # 3 пробела * 2 = 6 пробелов
...
# ============================================
# Параметризованные тесты (ИСПРАВЛЕННАЯ ВЕРСИЯ)
# ============================================
...
class TestRepeatParametrized:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("e", 0, ""),
            ("e", 3, "eee"),
            # ИСПРАВЛЕНО: правильный результат для " ABC " повторённого 2 раза
            (" ABC ", 2, " ABC  ABC "),  # было " ABC ABC "
            ("a", 1, "a"),
            ("", 5, ""),
            ("x", 10, "xxxxxxxxxx"),
            ("Hello", 2, "HelloHello"),
            ...
            ("123", 0, ""),
            ("\n", 2, "\n\n"),
            # Дополнительные тесты с пробелами
            ("ABC", 2, "ABCABC"),
            (" ABC", 2, " ABC ABC"),
            ("ABC ", 2, "ABC ABC "),
        ],
    ...
    def test_repeat_valid_cases(self, pattern, count, expected):
        ...
        assert (
            result == expected
        ), f"repeat('{pattern}', {count}) = '{result}', ожидалось '{expected}'"
...
def test_repeat_with_concatenation():
    ...
    part1 = repeat("AB", 2)  # ABAB
    part2 = repeat("CD", 3)  # CDCDCD
...
# ============================================
# Демонстрация с объяснением ошибки в спецификации
# ============================================


def demonstrate_repeat():
    """Демонстрация работы функции repeat с объяснением"""
    print("Демонстрация работы repeat:\n")

    print(f'repeat("e", 0) = "{repeat("e", 0)}"')
    print(f'repeat("e", 3) = "{repeat("e", 3)}"')

    # Показываем правильный результат
    pattern = " ABC "
    result = repeat(pattern, 2)
    print(f'\nПаттерн: "{pattern}" (длина {len(pattern)})')
    print(f'Паттерн * 2: "{pattern * 2}"')
    print(f'repeat("{pattern}", 2) = "{result}"')
    print(f"Длина результата: {len(result)}")
    print(f"\nОбъяснение:")
    print(f'  "{pattern}" + "{pattern}" = "{pattern + pattern}"')
    print(f'  В середине два пробела подряд: "{"  "}"')
    print(f"\n  Спецификация содержит ошибку:")
    print(f'  Спецификация говорит: " ABCABC "')
    print(f'  Правильный результат: " ABC  ABC "')

    try:
        repeat("e", -2)
    except ValueError as e:
        print(f'\nrepeat("e", -2) -> ValueError: {e}')
    ...
    print(f'\nrepeat("Hello", 3) = "{repeat("Hello", 3)}"')
    print(f'repeat("😀", 5) = "{repeat("😀", 5)}"')
    print(f'repeat("AB", 4) = "{repeat("AB", 4)}"')
