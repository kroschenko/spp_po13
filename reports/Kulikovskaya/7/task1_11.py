...
class RotatingRectangleApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Вращающийся прямоугольник")
    ...
    def create_ui(self):
        ...
        self._create_rotation_controls(control_frame)
        self._create_buttons(control_frame)

        # Canvas
        self.canvas = tk.Canvas(self.master, bg="white", highlightthickness=1,
    ...
    def _create_size_controls(self, parent):
        ...
        ttk.Label(parent, text="Ширина:").grid(row=0, column=0, padx=5)
        self.width_var = tk.IntVar(value=self.width)
        ttk.Spinbox(parent, from_=50, to=300, textvariable=self.width_var,
                    width=8).grid(row=0, column=1, padx=5)

        ttk.Label(parent, text="Высота:").grid(row=0, column=2, padx=5)
        self.height_var = tk.IntVar(value=self.height)
        ttk.Spinbox(parent, from_=50, to=300, textvariable=self.height_var,
                    width=8).grid(row=0, column=3, padx=5)

        ttk.Label(parent, text="Скорость:").grid(row=0, column=4, padx=5)
        self.speed_var = tk.IntVar(value=self.rotation_speed)
        ttk.Spinbox(parent, from_=1, to=20, textvariable=self.speed_var,
                    width=8).grid(row=0, column=5, padx=5)

    def _create_rotation_controls(self, parent):
        # Элементы управления вращением
        ttk.Label(parent, text="Точка вращения:").grid(row=1, column=0,
                                                        padx=5, pady=5)
        self.rotation_var = tk.StringVar(value=self.rotation_center)
        ttk.Combobox(parent, textvariable=self.rotation_var,
                     values=["top_left", "top_right", "bottom_left",
                             "bottom_right", "center"],
                     width=12, state="readonly").grid(row=1, column=1, padx=5,
                                                      pady=5)

        ttk.Button(parent, text="Цвет заливки",
                   command=self.choose_fill_color).grid(row=1, column=2, padx=5)
        ttk.Button(parent, text="Цвет контура",
                   command=self.choose_outline_color).grid(row=1, column=3,
                                                           padx=5)
    ...
    def _create_buttons(self, parent):
        ...
        ttk.Button(btn_frame, text="Скриншот",
    ...
    def choose_outline_color(self):
        color = colorchooser.askcolor(title="Выберите цвет контура",
    ...
    def get_rectangle_points(self, params):
        ...
        for x, y in corners:
            ...
            rotated_corners.append((params.cx + new_x, params.cy + new_y))

        return rotated_corners

    def get_rotation_point(self, corners):
        # Получить точку вращения
        if self.rotation_center == "top_left":
            return corners[0]
        if self.rotation_center == "top_right":
            return corners[1]
        if self.rotation_center == "bottom_right":
            return corners[2]
        if self.rotation_center == "bottom_left":
            return corners[3]
    ...
    def draw_rectangle(self):
        ...
        corners = self.get_rectangle_points(params)

        points = []
        for x, y in corners:
            points.extend([x, y])

        self.canvas.create_polygon(points, fill=self.fill_color,
                                   outline=self.outline_color, width=3)

        rot_point = self.get_rotation_point(corners)
        self.canvas.create_oval(rot_point[0] - 5, rot_point[1] - 5,
                                rot_point[0] + 5, rot_point[1] + 5,
                                fill="red", outline="darkred")

        self.canvas.create_text(self.center_x, 30,
                                text=f"Угол: {self.angle:.1f}°",
                                font=("Arial", 12))
        self.canvas.create_text(self.center_x, 50,
                                text=f"Точка вращения: {self.rotation_center}",
                                font=("Arial", 10))
