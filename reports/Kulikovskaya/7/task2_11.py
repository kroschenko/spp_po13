...
class JuliaSetApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Множество Жюлиа")
        ...
        # Инициализация всех UI-атрибутов
        self.c_real_var = None
        self.c_imag_var = None
    ...
    def create_ui(self):
        ...
        self._create_complex_controls(control_frame)
        ...
        self.canvas = tk.Canvas(canvas_frame, bg="black", width=self.width,
                                height=self.height, highlightthickness=1,
        ...
        # Информация
        self.info_label = ttk.Label(self.master, text="ЛКМ: перемещение",
                                     font=("Arial", 9))
    ...
    def _create_complex_controls(self, parent):
        # Параметры комплексного числа c
        ttk.Label(parent, text="c =").grid(row=0, column=0, padx=5)
        self.c_real_var = tk.DoubleVar(value=self.c_real)
        ttk.Entry(parent, textvariable=self.c_real_var, width=8).grid(row=0,
                                                                      column=1,
                                                                      padx=2)

        ttk.Label(parent, text="+").grid(row=0, column=2)
        self.c_imag_var = tk.DoubleVar(value=self.c_imag)
        ttk.Entry(parent, textvariable=self.c_imag_var, width=8).grid(row=0,
                                                                      column=3,
                                                                      padx=2)

        ttk.Label(parent, text="i").grid(row=0, column=4)

        # Пресеты
        ttk.Label(parent, text="Пресет:").grid(row=0, column=5, padx=10)
        self.preset_var = tk.StringVar(value="custom")
        presets = ttk.Combobox(parent, textvariable=self.preset_var,
                               values=["custom", "dendrite", "rabbit",
                                       "spiral", "galaxy"],
        ...
        # Итерации
        ttk.Label(parent, text="Итерации:").grid(row=0, column=7, padx=10)
        self.iter_var = tk.IntVar(value=self.max_iter)
        ttk.Spinbox(parent, from_=50, to=1000, textvariable=self.iter_var,
                    width=6).grid(row=0, column=8, padx=5)

    def _create_color_controls(self, parent):
        # Цветовая схема и кнопки
        ttk.Label(parent, text="Цвета:").grid(row=1, column=0, padx=5, pady=5)
        self.color_var = tk.StringVar(value=self.color_scheme)
        color_combo = ttk.Combobox(parent, textvariable=self.color_var,
        ...
        ttk.Button(btn_frame, text="Построить",
                   command=self.draw_julia).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Сбросить вид",
    ...
    def get_color(self, iteration, max_iter, **_):
    ...
    def draw_julia(self):
            ...
            if py % 20 == 0:
                try:
                    self.info_label.config(
                        text=f"Рисование... {100 * py // self.height}%")
                    self.master.update()
                except tk.TclError:
                    return
        ...
        try:
            self.info_label.config(
                text=f"c = {c_real} + {c_imag}i | Итерации: {max_iter} | Готово")
        except tk.TclError:
            pass
