...
class TourPackage: # pylint: disable=too-few-public-methods
...
if __name__ == "__main__":
    ...
    selected_transport, selected_transport_cost = choose_option(
        transport_options, "Выберите транспорт:"
    )
    builder.set_transport(selected_transport, selected_transport_cost)

    accommodation_options = {"Хостел": 50, "Отель 3*": 200, "Отель 5*": 500}
    selected_accommodation, selected_accommodation_cost = choose_option(
        accommodation_options, "Выберите проживание:"
    )
    ...
    while True:
        for index, (exc_name, exc_cost) in enumerate(excursions_options.items(), 1):
            print(f"{index}. {exc_name} — {exc_cost} USD")
